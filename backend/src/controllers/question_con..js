...
exports.createQuestion = async (req, res) => {
  try {
    ...
    const newQuestion = new Question({
      ...
      description,
      tags,
      posted_by: req.real_user._id, // from auth middleware
    });
...
exports.getAllQuestions = async (req, res) => {
  try {
    ...
    const questions = await Question.find()
      .populate('posted_by', 'first_name profile_pic_url',) // Fetch username from User model
...
exports.getQuestionById = async (req, res) => {
  try {
    const question = await Question.findById(req.params.id)
      .populate('posted_by', 'first_name profile_pic_url')
      .populate({
        path: 'answers',
        populate: {
          path: 'posted_by',
...
exports.deleteQuestion = async (req, res) => {
  try {
    ...
    if (question.posted_by.toString() !== req.real_user._id.toString()) {
